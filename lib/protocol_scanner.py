...
def shodan_search(query: str, protocol: str) -> int:
    ...
    for result in results['matches']:
        try:
            ...
            try:
                with open(output_file, "a", encoding="utf-8") as ips:
                    ips.write(ip)
                    ips.write("\n")
            except IOError:
            ...
            break
    print(f"\nResults saved under: {output_file}")
    print("Finished")
    return 1


def censys_search(query: str, protocol: str) -> int:
    print("Extracting IPs for following query: " + query)
    print("Please wait..")
    discovery_id = id_generator()
    pages = 2
    ...
    while page <= pages:
        try:
            print(f"Extracting IPs from page {page}")
            ...
            break
    print(f"Results saved under: {output_file}")
    print("Finished")
    return 1



def mass_scan(ip: str, protocol: str) -> int:
    ...
    if not res:
        infile = open("masscan.xml", "r", encoding="utf-8")
