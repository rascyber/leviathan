...
def google_search(search_term: str, api_key: str, cse_id: str, **kwargs) -> list:
    service = build("customsearch", "v1", developerKey=api_key)
    res = service.cse().list(q=search_term, cx=cse_id, **kwargs).execute()
    return res['items']


def link_extract(query: str, number: int) -> None:
    try:
        ...
        for result in results:
            ...
            with open(filename, "a", encoding="utf-8") as links:
...
def sqli_scan(discovery_id: str) -> None:
    discovered_file = os.path.join(BASE_DIR, 'assets', 'discovered', 'google_web_' + str(discovery_id) + '.txt')
    output_file = os.path.join(BASE_DIR, 'assets', 'compromised', f'google_web_{discovery_id}.txt')
    try:
        with open(discovered_file, "r", encoding="utf-8") as urls_file:
        ...
        print(f"There is no such file: {discovered_file}")
    print("\n")
...
@timeout()
def sqli_scan_by_url(url: str, output_file: str) -> None:
    print("Trying: " + url)
    ...
    for line in unique_lines:
        try:
            ...
            print("\n")
            with open(output_file, "a", encoding="utf-8") as cracked:
                cracked.write(line)
                cracked.write("\n")
        except IOError:
            print(f"There is no such file: {output_file}")



def sqli_scan_all() -> None:
